...
#End Region
    ...
    Public Sub SendHTMLMail()
        ...
        Try
            ...
            ' myString = myString.Replace("$$Date1$$", Trim(SS))
            ...
            '    I1 = I1 + 1
            '    I2 = I2 + Val(DR.GetValue(2))
            '    SS = SS + Trim(DR.GetValue(0)) + " " + Trim(DR.GetValue(1)) + " "


            '    End While

            '    myString = myString.Replace("$$Cont$$", Trim(SS))
            ...
            ''Msg.[To].Add(New MailAddress("palithan@samsoncorporates.com"))
            ' Msg.[To].Add(New MailAddress("palithan@samsoncorporates.com;it@samsoncorporates.com;palithan@samsoncorporates.com"))

            ' Subject of e-mail
            ''Msg.Subject = "Arival Notice  ETA: " + Mid(Trim(TxtEta.Text), 7, 4) + "-" + Mid(Trim(TxtEta.Text), 4, 2) + "-" + Mid(Trim(TxtEta.Text), 1, 2)
            ...
            a.Credentials = New Net.NetworkCredential("insurance@samsoncorporates.com", "PKM#mLo%957")
            a.Port = 587
            a.EnableSsl = True
            ...
            ' htmlContent.AppendLine("<link rel='stylesheet' href='StyleHtml.css' media='all' />")
            htmlContent.AppendLine("<style>")
            htmlContent.AppendLine("        body { background-color: #f0f0f0; font-family: Calibri, Calibri; }")
            'htmlContent.AppendLine("        body { background-color: #f0f0f0; font-family: Calibri, sans-serif; }")
            ' htmlContent.AppendLine("        h1 { color: #007bff; }")
            'htmlContent.AppendLine("        body { background-color: #5D6975; font-family: Calibri; font-size: 10px; ")
            ...
            'htmlContent.AppendLine("h1 { border-top: 1px solid  #5D6975; border-bottom: 1px solid  #5D6975 color: #5D6975; font-size: 2.4em; font-weight: normal; text-align: center; margin: 0 0 20px 0; background: url('https://drive.google.com/uc? export=view&id=1JDrL53pr2cfExu5t3o4gZrmz-2scb182'); }")
            'htmlContent.AppendLine("        body { background-color: #FFFFFF; font-size: 12px; font-family: Arial; }")


            ''htmlContent.AppendLine(".clearfix:after { content: ""; display: table; clear: both; }")
            'htmlContent.AppendLine("content: "";")
            'htmlContent.AppendLine("display: table;")
            'htmlContent.AppendLine("clear: both;")
            'htmlContent.AppendLine("}")

            ''htmlContent.AppendLine("a { color: #5D6975; text-decoration: underline; }")
            'htmlContent.AppendLine("color: #5D6975;")
            'htmlContent.AppendLine("text-decoration: underline;")
            'htmlContent.AppendLine("}")

            '' htmlContent.AppendLine("        body { position: relative; width: 21cm; height: 29.7cm; margin: 0 auto; color: #001028; background: #FFFFFF; font-size: 12px; font-family: Arial; }")
            'htmlContent.AppendLine("position: relative;")
            'htmlContent.AppendLine("width: 21cm;")
            'htmlContent.AppendLine("height: 29.7cm;")
            'htmlContent.AppendLine("margin: 0 auto;")
            'htmlContent.AppendLine("color: #001028;")
            'htmlContent.AppendLine("background: #FFFFFF;")
            'htmlContent.AppendLine("font-family: Arial, sans-serif;")
            'htmlContent.AppendLine("font-size: 12px;")
            'htmlContent.AppendLine("font-family: Arial;")
            'htmlContent.AppendLine("}")

            ''htmlContent.AppendLine("header { padding: 10px 0; margin-bottom: 30px; }")
            'htmlContent.AppendLine("padding: 10px 0;")
            'htmlContent.AppendLine("margin-bottom: 30px;")
            'htmlContent.AppendLine("}")

            '' htmlContent.AppendLine("#logo { text-align: center; margin-bottom: 10px; }")
            'htmlContent.AppendLine("text-align: center;")
            'htmlContent.AppendLine("margin-bottom: 10px;")
            'htmlContent.AppendLine("}")

            ''htmlContent.AppendLine("#logo img { width: 90px; }")
            'htmlContent.AppendLine(" width: 90px;")
            'htmlContent.AppendLine("}")

            ''htmlContent.AppendLine("h1 { border-top: 1px solid  #5D6975; border-bottom: 1px solid  #5D6975 color: #5D6975; font-size: 2.4em; font-weight: normal; text-align: center; margin: 0 0 20px 0; background: url('https://drive.google.com/uc? export=view&id=1JDrL53pr2cfExu5t3o4gZrmz-2scb182'); }")
            'htmlContent.AppendLine("border-top: 1px solid  #5D6975;")
            'htmlContent.AppendLine("border-bottom: 1px solid  #5D6975;")
            'htmlContent.AppendLine("color: #5D6975;")
            'htmlContent.AppendLine("font-size: 2.4em;")
            'htmlContent.AppendLine("line-height: 1.4em;")
            'htmlContent.AppendLine("font-weight: normal;")
            'htmlContent.AppendLine("text-align: center;")
            'htmlContent.AppendLine("margin: 0 0 20px 0;")
            'htmlContent.AppendLine("background: url(dimension.png);")
            'htmlContent.AppendLine("}")

            ''htmlContent.AppendLine("#project { float: left; }")
            'htmlContent.AppendLine("float: left;")
            'htmlContent.AppendLine("}")

            ''htmlContent.AppendLine("#project span { color: #5D6975; text-align: right;  width: 52px; margin-right: 10px; display: inline-block; font-size: 0.8em; }")
            'htmlContent.AppendLine("color: #5D6975;")
            'htmlContent.AppendLine("text-align: right;")
            'htmlContent.AppendLine(" width: 52px;")
            'htmlContent.AppendLine(" margin-right: 10px;")
            'htmlContent.AppendLine("display: inline-block;")
            'htmlContent.AppendLine("font-size: 0.8em;")
            'htmlContent.AppendLine("}")

            ''htmlContent.AppendLine("#company { float: right; text-align: right; }")
            'htmlContent.AppendLine(" float: right;")
            'htmlContent.AppendLine(" text-align: right;")
            'htmlContent.AppendLine("}")

            ''htmlContent.AppendLine("#project div,#company div {white-space: nowrap; }")
            'htmlContent.AppendLine("#company div {")
            'htmlContent.AppendLine(" white-space: nowrap;")
            'htmlContent.AppendLine("}")

            ''htmlContent.AppendLine("table { width: 100%; border-collapse: collapse; border-spacing: 0; margin-bottom: 20px;}")
            'htmlContent.AppendLine(" width: 100%;")
            'htmlContent.AppendLine(" border-collapse: collapse;")
            'htmlContent.AppendLine(" border-spacing: 0;")
            'htmlContent.AppendLine(" margin-bottom: 20px;")
            'htmlContent.AppendLine("}")

            ''htmlContent.AppendLine("table tr:nth-child(2n-1) td { background: #F5F5F5;}")
            'htmlContent.AppendLine(" background: #F5F5F5;")
            'htmlContent.AppendLine("}")

            ''htmlContent.AppendLine("table th,table td {text-align: center;}")
            'htmlContent.AppendLine("table td {")
            'htmlContent.AppendLine("text-align: center;")
            'htmlContent.AppendLine("}")

            ''htmlContent.AppendLine("table th { padding: 5px 20px;color: #5D6975;border-bottom: 1px solid #C1CED9;white-space: nowrap;font-weight: normal;}")
            'htmlContent.AppendLine(" padding: 5px 20px;")
            'htmlContent.AppendLine("color: #5D6975;")
            'htmlContent.AppendLine("border-bottom: 1px solid #C1CED9;")
            'htmlContent.AppendLine("white-space: nowrap;")
            'htmlContent.AppendLine("font-weight: normal;")
            'htmlContent.AppendLine("}")

            ''htmlContent.AppendLine("table .service,table .desc {text-align: left;}")
            'htmlContent.AppendLine("table .desc {")
            'htmlContent.AppendLine("text-align: left;")
            'htmlContent.AppendLine("}")

            ''htmlContent.AppendLine("table td {padding: 20px;text-align: right;}")
            'htmlContent.AppendLine("padding: 20px;")
            'htmlContent.AppendLine("text-align: right;")
            'htmlContent.AppendLine("}")

            ''htmlContent.AppendLine("table td.service,table td.desc {vertical-align: top;}")
            'htmlContent.AppendLine("table td.desc {")
            'htmlContent.AppendLine("vertical-align: top;")
            'htmlContent.AppendLine("}")

            ''htmlContent.AppendLine("table td.unit,table td.qty,table td.total {font-size: 1.2em;}")
            'htmlContent.AppendLine("table td.qty,")
            'htmlContent.AppendLine("table td.total {")
            'htmlContent.AppendLine("font-size: 1.2em;")
            'htmlContent.AppendLine("}")

            '' htmlContent.AppendLine("table td.grand {border-top: 1px solid #5D6975;;}")
            'htmlContent.AppendLine("border-top: 1px solid #5D6975;;")
            'htmlContent.AppendLine("}")

            '' htmlContent.AppendLine("#notices.notice {color: #5D6975;font-size: 1.2em;}")
            'htmlContent.AppendLine("color: #5D6975;")
            'htmlContent.AppendLine("font-size: 1.2em;")
            'htmlContent.AppendLine("}")

            '' htmlContent.AppendLine("footer {color: #5D6975;width: 100%;height: 30px;position: absolute;bottom: 0;border-top: 1px solid #C1CED9;padding: 8px 0;text-align: center;}")
            'htmlContent.AppendLine("color: #5D6975;")
            'htmlContent.AppendLine("width: 100%;")
            'htmlContent.AppendLine("height: 30px;")
            'htmlContent.AppendLine("position: absolute;")
            'htmlContent.AppendLine("bottom: 0;")
            'htmlContent.AppendLine("border-top: 1px solid #C1CED9;")
            'htmlContent.AppendLine("padding: 8px 0;")
            'htmlContent.AppendLine("text-align: center;")
            'htmlContent.AppendLine("}")
            ...
            'htmlContent.AppendLine("<table border='1' style=font-family: 'Calibri', Calibri; font-size: 8px; border-collapse: collapse; width: 100%; text-align: left;>")
            ...
            htmlContent.AppendLine("<td style='background-color: #0000ff; color: #ffffff;'>Refno  </td>")
            htmlContent.AppendLine("<td style='background-color: #0000ff; color: #ffffff;'>PolicyDate</td>")
            htmlContent.AppendLine("<td style='background-color: #0000ff; color: #ffffff;'>PolicyNo </td>")
            htmlContent.AppendLine("<td style='background-color: #0000ff; color: #ffffff;'>DnNo    </td>")
            htmlContent.AppendLine("<td style='background-color: #0000ff; color: #ffffff;'>Type  </td>")
            htmlContent.AppendLine("<td style='background-color: #0000ff; color: #ffffff;'>Customer            </td>")
            htmlContent.AppendLine("<td style='background-color: #0000ff; color: #ffffff;'>Company             </td>")
            ...
            'CON.ConnectionString = ("data source=(local);initial catalog=ImpShp; user id=sa;password=tstc123")
            Con1.Open()
            ADA = New SqlDataAdapter("SELECT ID,Refno,PolDt,Customer,PolicyNo,DNNo,Company,TYPE,PREMIUM from Refno where PaidY<>'Y' order by PolDt ", Con1)
            ...
            a1.Credentials = New Net.NetworkCredential("insurance@samsoncorporates.com", "PKM#mLo%957")
            a1.Port = 587
            a1.EnableSsl = True
